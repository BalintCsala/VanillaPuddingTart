...
Material getMaterial(ivec3 blockPos, vec2 texCoord) {
    bool inside;
    ivec2 cell = positionToCell(vec3(blockPos), inside);
    ivec2 pixel = cellToPixel(cell);
    ...
    return Material(
        albedo,
        normalize(vec3(normalData.xy, sqrt(1.0 - dot(normalData.xy, normalData.xy)))),
        normalData.z,
...
Hit raytrace(inout Ray ray) {
    ...
    for (int i = 0; i < MAX_RT_STEPS; i++) {
        ...
        if (isSolid(nextCell)) {
            ...
            Hit hit = Hit(true, t, normal, getMaterial(nextCell, texCoord));
            return hit;
        }
...
bool checkSun(vec3 hitPos, ivec3 hitCell) {
    Ray sunRay = Ray(hitPos, hitCell, sunDir);
...
vec3 pathtrace(vec3 playerPos, vec3 normal) {
    ...
    Material data = getMaterial(ivec3(floor(playerPos - fract(chunkOffset) - normal * 0.1)), texCoord);
    
    vec3 radiance = data.albedo * data.emission;
    vec3 throughput = vec3(1.0);
    
    vec3 rayDir;
    if (data.metal) {
        rayDir = normalize(reflect(playerPos, normal));
    } else {
        rayDir = cosineWeighted(normal);
    }
    throughput *= fresnel(data.F0, max(dot(-rayDir, normal), 0.0));
    
    Ray ray = Ray(playerPos - fract(chunkOffset), ivec3(floor(playerPos - fract(chunkOffset))), rayDir);
    
    if (!data.metal && checkSun(ray.origin, ray.cell)) {
        radiance += max(dot(normal, sunDir), 0.0) / PI * throughput * 8.0;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        radiance += hit.data.emission * hit.data.albedo * throughput * 4.0;
        throughput *= fresnel(hit.data.F0, max(dot(ray.direction, hit.geometryNormal), 0.0));
        
        throughput *= hit.data.albedo;
        
        
        if (!hit.data.metal && checkSun(ray.origin, ray.cell)) {
            radiance += max(dot(hit.geometryNormal, sunDir), 0.0) / PI * throughput * 8.0;
        }
        
        if (hit.data.metal) {
            ray.direction = normalize(reflect(ray.direction, hit.geometryNormal));
        } else {
            ray.direction = cosineWeighted(hit.geometryNormal);
        }
    }
    return radiance;
}
...
void main() {
    ...
    vec3 normal = getNormal(texCoord, depth, viewPos);
    vec3 radiance = pathtrace(playerPos, normal) * albedo;
    
    fragColor.rgb = uchimura(radiance);
    vec3 outputColor = pow(fragColor.rgb, vec3(1.0 / GAMMA_CORRECTION));
